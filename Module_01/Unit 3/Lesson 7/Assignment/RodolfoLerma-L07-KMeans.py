...
Binary question:
     ...
     Does the horsepower provide an indication on the price of the car?
         Based on the clustering analysis yes, even though the correlation is not one to one
         the more power the engine has the more likely the price of the car will increase. 
         But by following the data (see plot below) it is interesting to notice that after
         certain horsepower range the price of the car lowers back a little bit.
     
Non Binary question:
    How many groups could be seen (if any) using the highway-mpg and num of cylinders attributes?
    When does scatter analysis shows any grouping or differences for the num of cylinders and highway mpg?
        At least 2 clusters are clearly seen, but one could argue than maybe more are present.
        From 2 clusters it seems that cars with lower number of cylinders get more Highway mpg, but not in a 
        one to one correlation but in a range.
...
auto.loc[auto.loc[:, "num-of-cylinders"] == "eight", "num-of-cylinders"] = 6
auto.loc[auto.loc[:, "num-of-cylinders"] == "five", "num-of-cylinders"] = 4
auto.loc[auto.loc[:, "num-of-cylinders"] == "four", "num-of-cylinders"] = 3
auto.loc[auto.loc[:, "num-of-cylinders"] == "six", "num-of-cylinders"] = 5
auto.loc[auto.loc[:, "num-of-cylinders"] == "three", "num-of-cylinders"] = 2
auto.loc[auto.loc[:, "num-of-cylinders"] == "twelve", "num-of-cylinders"] = 7
auto.loc[auto.loc[:, "num-of-cylinders"] == "two", "num-of-cylinders"] = 1
...
auto["num-of-cylinders" + names] = (auto["num-of-cylinders"] - np.mean(auto["num-of-cylinders"]))/np.std(auto["num-of-cylinders"]) #mean and std dev
...
second_data_set = auto[["num-of-cylinders_norm", "highway-mpg_norm"]] #Selecting just 2 attributes from the data (the values are normalized with respect its mean and std dev)
...
#    plt.scatter(second_data_set["num-of-cylinders_norm"], second_data_set["highway-mpg_norm"], c=labels, alpha=a, s=s, edgecolors=ec)
#    plt.scatter(centers[:,0], centers[:,1], marker="*", color='r')
#    plt.title('N_Clusters = ' + str(i))
#    plt.xlabel("num-of-cylinders_norm")
#    plt.ylabel("highway-mpg_norm")
#    plt.show()
    
#based on the previous data the number of clusters for each set of data
   
k_means = KMeans(n_clusters=4) #Number of clusters decided based on previous plots (look at the lines commented above)
values = k_means.fit(first_data_set)
labels = values.labels_
auto["horsepower_price_label"] = labels  #storing the label on the dataset
center = values.cluster_centers_ #center of the cluster
centers = np.array(center) #transform to an numpy array to later use it on the plot
...
plt.scatter(centers[:,0], centers[:,1], marker="*", color='r') #plot the center for each cluster
plt.title('N_Clusters = ' + str(4))
plt.xlabel("horsepower_norm")
plt.ylabel("price_norm")
plt.show()

k_means = KMeans(n_clusters=2) #Number of clusters decided based on previous plots (look at the lines commented above)
values = k_means.fit(second_data_set)
labels = values.labels_
labels = values.labels_
auto["cylinders_highway_label"] = labels  #storing the label on the dataset
center = values.cluster_centers_ #center of the cluster
centers = np.array(center) #transform to an numpy array to later use it on the plot
plt.scatter(second_data_set["num-of-cylinders_norm"], second_data_set["highway-mpg_norm"], c=labels, alpha=a, s=s, edgecolors=ec)  #plot the cloud of points with clear distinction of the clusters
plt.scatter(centers[:,0], centers[:,1], marker="*", color='r') #plot the center for each cluster
